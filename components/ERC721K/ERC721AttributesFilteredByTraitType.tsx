...
export const ERC721AttributesFilteredByTraitType = ({
  ...
  const containerClassName = classNames(className, 'ERC721AttributesFilteredByTraitType')

  const [filteredAttributes, setFilteredAttributes] = React.useState<any[]>()
  React.useEffect(() => {
    if (Array.isArray(attributes) && Array.isArray(attFilter)) {
      const boostNumbers = attributes.filter((attribute) => attFilter.includes(attribute.trait_type))
