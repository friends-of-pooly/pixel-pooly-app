...
export const ERC721AttributesBoostNumbers = ({
  ...
  const containerClassName = classNames(className, 'ERC721AttributesBoostNumbers')

  const [filteredAttributes, setFilteredAttributes] = React.useState<any[]>()
  React.useEffect(() => {
    if (Array.isArray(attributes)) {
      const boostNumbers = attributes.filter((attribute) => attribute.display_type === 'boost_number')
